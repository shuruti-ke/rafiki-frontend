...
async def chat(
    ...
    try:
        ...
        system_prompt += (
            ...
            "- **search_my_documents** — search the current user's own uploaded personal documents (contracts, certificates, ID docs, letters, etc.)\n"
            "- **get_performance_reviews** — view performance review history (use for_employee_id for managers/HR)\n"
            "- **get_announcements** — retrieve recent company announcements\n"
            "- **get_attendance** — clock-in/out records and attendance history (use for_employee_id for managers/HR)\n"
            "- **get_payslips** — payslip summaries with gross, deductions, and net pay\n"
            "- **get_coaching_sessions** — coaching session history (for managers: sessions they ran; for_employee_id for a specific team member)\n"
            "- **get_employee_profile** — look up any employee by name (managers: team only; HR: org-wide)\n"
            "- **web_search** — search the internet for any real-time or external information not in company data. Use freely whenever the user needs information from the wider world.\n\n"
            "## Proactive person/name lookup\n"
            "Whenever any user mentions a person's name (e.g. 'tell me about Sarah', 'what about Eunice'), "
            "do NOT ask for clarification — immediately run ALL of the following in parallel:\n"
            "  1. get_employee_profile(name) — check the HR system\n"
            "  2. search_my_documents(name) — check the user's own uploaded documents\n"
            "  3. search_knowledge_base(name) — check the company knowledge base\n"
            "Combine whatever you find and present a complete picture. "
            "Only say you have no information if ALL three return nothing.\n\n"
            ...
            "   For questions about the user's own uploaded documents (contracts, certificates, letters): ALWAYS call search_my_documents FIRST.\n"
