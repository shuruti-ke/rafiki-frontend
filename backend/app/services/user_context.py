...
def _safe_rollback(db: Session) -> None:
    """Rollback helper for swallowed DB errors so later queries can still run."""
    try:
        db.rollback()
    except Exception:
        pass
...
def _find_report_user_id_by_name(
    ...
    """
    Resolve a direct report's user_id by name, but only within the current
    manager's active direct reports in the current org.
    """
    try:
        direct_reports = _get_direct_reports(db, org_id, manager_id)
        name_lower = (name or '').strip().lower()

        logger.info(
            "🔍 HYBRID LOOKUP: Finding user_id for name='%s' manager='%s' org='%s'",
            name,
            manager_id,
            org_id,
        )
        ...
        logger.info("📊 Found %d active direct reports", len(direct_reports))

        if not direct_reports:
            logger.warning(
                "❌ No active direct reports exist for manager_id='%s' in org='%s'",
                manager_id,
                org_id,
            )
            return None
        ...
        for report in direct_reports:
            report_name = (getattr(report, 'name', None) or '').strip().lower()
            if report_name == name_lower:
                logger.info("✅ EXACT MATCH FOUND: '%s' → user_id=%s", report.name, report.user_id)
                return report.user_id

        for report in direct_reports:
            report_name = (getattr(report, 'name', None) or '').strip().lower()
            if report_name.startswith(name_lower):
                logger.info("✅ PREFIX MATCH FOUND: '%s' → user_id=%s", report.name, report.user_id)
                return report.user_id
            if name_lower in report_name.split():
                logger.info("✅ TOKEN MATCH FOUND: '%s' → user_id=%s", report.name, report.user_id)
                return report.user_id
            if name_lower in report_name:
                logger.info("✅ PARTIAL MATCH FOUND: '%s' → user_id=%s", report.name, report.user_id)
                return report.user_id
...
def _build_direct_report_context(
    ...
    """
    try:
        # Earlier optional builders may have swallowed a DB error and left the
        # session in an aborted transaction state. Clear it before direct-report
        # lookups so the name resolver gets a fair shot.
        _safe_rollback(db)
