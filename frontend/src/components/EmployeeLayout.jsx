...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    links: [
      ...
      { to: "/my-shifts", label: "My Shifts", icon: "🗓️" },
      { to: "/my-workflows", label: "My Workflows", icon: "✅" },
...
function ObjectiveReviewBlock({ payload, onAction }) {
  ...
  useEffect(() => {
    ...
    authFetch(`${API}/api/v1/objectives/${payload.objective_id}`)
      ...
      .then(obj => {
        if (!obj) return;
        if (obj.status === "active") setDone("approved");
        else if (obj.status === "draft" && obj.review_status === "needs_revision") setDone("needs_revision");
        else if (obj.status === "cancelled") setDone("cancelled");
      })
      .catch(() => {})
...
function MiniCalendar() {
  ...
  const firstDay = new Date(year, month, 1).getDay(); /* Sunday-first */
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const cells = [];
  for (let i = 0; i < firstDay; i++) cells.push(null);
  for (let d = 1; d <= daysInMonth; d++) cells.push(d);
  const rem = 7 - (cells.length % 7);
  if (rem < 7) for (let i = 1; i <= rem; i++) cells.push(null);
...
export default function EmployeeLayout() {
  ...
  return (
    <div className="emp-layout">
      ...
      {/* ── Sidebar nav ── */}
      <nav className={`emp-sidebar-nav${collapsed ? " collapsed" : ""}${mobileOpen ? " mobile-open" : ""}`} style={{ position: "relative" }}>
      ...
      <div className="emp-main">
        ...
        <div className="emp-topbar">
          ...
          <div className="emp-topbar-actions">
            <NotificationBell />
            <button className="emp-topbar-icon-btn" onClick={(e) => { e.stopPropagation(); setMsgPanelOpen(o => !o); }} title="Messages">
              💬
      ...
      <div className={`emp-msg-panel${msgPanelOpen ? " open" : ""}`}>
        ...
        <div className="emp-msg-panel-body">
